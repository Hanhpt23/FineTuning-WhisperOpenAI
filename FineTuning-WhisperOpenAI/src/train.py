...
def train_whisper_model(args):
    ...
    #     # Filter out empty strings from label_str and their corresponding pred_str
    #     filtered_labels_and_preds = [(label, pred) for label, pred in zip(label_str, pred_str) if label]
    #     if not filtered_labels_and_preds:
    #         raise ValueError("All reference strings are empty")

    #     label_str, pred_str = zip(*filtered_labels_and_preds)

    #     wer = 100 * metric.compute(predictions=list(pred_str), references=list(label_str))
